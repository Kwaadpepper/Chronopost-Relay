...
class RechercheBtAvecPFParCodeproduitEtCodepostalEtDateResponse extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \ChronopostRelay\StructType\BureauDeTabacAvecPF[]
     */
     ...
     * @param \ChronopostRelay\StructType\BureauDeTabacAvecPF[] $return
     */
    ...
    /**
     * Get return value
     * @return \ChronopostRelay\StructType\BureauDeTabacAvecPF[]
     */
    ...
    public static function validateReturnForArrayConstraintFromSetReturn(?array $values = []): string
    {
        ...
        foreach ($values as $rechercheBtAvecPFParCodeproduitEtCodepostalEtDateResponseReturnItem) {
            ...
            if (!$rechercheBtAvecPFParCodeproduitEtCodepostalEtDateResponseReturnItem instanceof \ChronopostRelay\StructType\BureauDeTabacAvecPF) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The return property can only contain items of type \ChronopostRelay\StructType\BureauDeTabacAvecPF, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \ChronopostRelay\StructType\BureauDeTabacAvecPF[] $return
     * @return \ChronopostRelay\StructType\RechercheBtAvecPFParCodeproduitEtCodepostalEtDateResponse
     */
     ...
     * @throws InvalidArgumentException
     * @param \ChronopostRelay\StructType\BureauDeTabacAvecPF $item
     * @return \ChronopostRelay\StructType\RechercheBtAvecPFParCodeproduitEtCodepostalEtDateResponse
     */
    public function addToReturn(\ChronopostRelay\StructType\BureauDeTabacAvecPF $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \ChronopostRelay\StructType\BureauDeTabacAvecPF) {
            throw new InvalidArgumentException(sprintf('The return property can only contain items of type \ChronopostRelay\StructType\BureauDeTabacAvecPF, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->return[] = $item;
