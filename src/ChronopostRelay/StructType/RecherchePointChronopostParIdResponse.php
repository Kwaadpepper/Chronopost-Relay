...
class RecherchePointChronopostParIdResponse extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \ChronopostRelay\StructType\PointChronopost[]
     */
     ...
     * @param \ChronopostRelay\StructType\PointChronopost[] $return
     */
    ...
    /**
     * Get return value
     * @return \ChronopostRelay\StructType\PointChronopost[]
     */
    ...
    public static function validateReturnForArrayConstraintFromSetReturn(?array $values = []): string
    {
        ...
        foreach ($values as $recherchePointChronopostParIdResponseReturnItem) {
            ...
            if (!$recherchePointChronopostParIdResponseReturnItem instanceof \ChronopostRelay\StructType\PointChronopost) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The return property can only contain items of type \ChronopostRelay\StructType\PointChronopost, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \ChronopostRelay\StructType\PointChronopost[] $return
     * @return \ChronopostRelay\StructType\RecherchePointChronopostParIdResponse
     */
     ...
     * @throws InvalidArgumentException
     * @param \ChronopostRelay\StructType\PointChronopost $item
     * @return \ChronopostRelay\StructType\RecherchePointChronopostParIdResponse
     */
    public function addToReturn(\ChronopostRelay\StructType\PointChronopost $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \ChronopostRelay\StructType\PointChronopost) {
            throw new InvalidArgumentException(sprintf('The return property can only contain items of type \ChronopostRelay\StructType\PointChronopost, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->return[] = $item;
